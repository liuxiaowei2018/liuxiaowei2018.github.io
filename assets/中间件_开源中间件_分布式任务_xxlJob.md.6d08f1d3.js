import{_ as s,o as a,c as n,Q as l}from"./chunks/framework.8048b864.js";const u=JSON.parse('{"title":"xxlJob","description":"","frontmatter":{},"headers":[],"relativePath":"中间件/开源中间件/分布式任务/xxlJob.md","filePath":"中间件/开源中间件/分布式任务/xxlJob.md","lastUpdated":null}'),p={name:"中间件/开源中间件/分布式任务/xxlJob.md"},o=l(`<h1 id="xxljob" tabindex="-1">xxlJob <a class="header-anchor" href="#xxljob" aria-label="Permalink to &quot;xxlJob&quot;">​</a></h1><nav class="table-of-contents"><ul><li><a href="#_1、xxljob应用">1、XxlJob应用</a><ul><li><a href="#创建执行器">创建执行器</a></li></ul></li><li><a href="#_2、xxljob扩展">2、XxlJob扩展</a><ul><li><a href="#源码结构">源码结构</a></li><li><a href="#数据库结构">数据库结构</a></li><li><a href="#调度中心配置">调度中心配置</a></li></ul></li></ul></nav><h2 id="_1、xxljob应用" tabindex="-1">1、XxlJob应用 <a class="header-anchor" href="#_1、xxljob应用" aria-label="Permalink to &quot;1、XxlJob应用&quot;">​</a></h2><blockquote><p>官方文档：<a href="https://www.xuxueli.com/xxl-job" target="_blank" rel="noreferrer">https://www.xuxueli.com/xxl-job</a></p></blockquote><h3 id="创建执行器" tabindex="-1">创建执行器 <a class="header-anchor" href="#创建执行器" aria-label="Permalink to &quot;创建执行器&quot;">​</a></h3><p>1.创建一个简单的SpringBoot项目，引入xxlJob公共依赖。</p><div class="language-xml vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">xml</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#6A737D;">&lt;!-- xxlJob公共依赖 --&gt;</span></span>
...
<span class="line"><span style="color:#6A737D;">### 执行器日志文件保存天数 [选填] ： 过期日志自动清理, 限制值大于等于3时生效; 否则, 如-1, 关闭自动清理功能；</span></span>
<span class="line"><span style="color:#D73A49;">xxl.job.executor.logretentiondays</span><span style="color:#24292E;">=3</span></span></code></pre></div><h2 id="_2、xxljob扩展" tabindex="-1">2、XxlJob扩展 <a class="header-anchor" href="#_2、xxljob扩展" aria-label="Permalink to &quot;2、XxlJob扩展&quot;">​</a></h2><h3 id="源码结构" tabindex="-1">源码结构 <a class="header-anchor" href="#源码结构" aria-label="Permalink to &quot;源码结构&quot;">​</a></h3><blockquote><ol><li><code>xxl-job-admin：调度中心</code></li><li><code>xxl-job-core：公共依赖</code></li><li><code>xxl-job-executor-samples：执行器Sample示例（选择合适的版本执行器，可直接使用，也可以参考其并将现有项目改造成执行器）</code></li><li><code> ：xxl-job-executor-sample-springboot：Springboot版本，通过Springboot管理执行器，推荐这种方式；</code></li><li><code> ：xxl-job-executor-sample-frameless：无框架版本；</code></li></ol></blockquote><h3 id="数据库结构" tabindex="-1">数据库结构 <a class="header-anchor" href="#数据库结构" aria-label="Permalink to &quot;数据库结构&quot;">​</a></h3><blockquote><p>xxl_job_lock：任务调度锁表； xxl_job_group：执行器信息表，维护任务执行器信息； xxl_job_info：调度扩展信息表： 用于保存XXL-JOB调度任务的扩展信息，如任务分组、任务名、机器地址、执行器、执行入参和报警邮件等等； xxl_job_log：调度日志表： 用于保存XXL-JOB任务调度的历史信息，如调度结果、执行结果、调度入参、调度机器和执行器等等； xxl_job_log_report：调度日志报表：用户存储XXL-JOB任务调度日志的报表，调度中心报表功能页面会用到； xxl_job_logglue：任务GLUE日志：用于保存GLUE更新历史，用于支持GLUE的版本回溯功能； xxl_job_registry：执行器注册表，维护在线的执行器和调度中心机器地址信息； xxl_job_user：系统用户表；</p></blockquote><h3 id="调度中心配置" tabindex="-1">调度中心配置 <a class="header-anchor" href="#调度中心配置" aria-label="Permalink to &quot;调度中心配置&quot;">​</a></h3><div class="language-properties vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">properties</span><pre class="shiki github-dark vp-code-dark"><code><span class="line"><span style="color:#6A737D;">### 调度中心JDBC链接</span></span>
...
<span class="line"><span style="color:#24292E;">    </span></span>
<span class="line"><span style="color:#6A737D;">### 调度中心日志表数据保存天数 [必填]：过期日志自动清理；限制大于等于7时生效，否则, 如-1，关闭自动清理功能；</span></span>
<span class="line"><span style="color:#D73A49;">xxl.job.logretentiondays</span><span style="color:#24292E;">=30</span></span></code></pre></div><blockquote><p>调度中心访问地址：<a href="https://link.juejin.cn/?target=http%3A%2F%2F127.0.0.1%3A8080%2Fxxl-job-admin" target="_blank" rel="noreferrer">http://127.0.0.1:8080/xxl-job-admin</a> (<strong>该地址执行器将会使用到，作为回调地址</strong>)</p><p>默认登录账号：&quot;admin/123456&quot;</p></blockquote>`,17),e=[o];function t(c,r,i,y,x,E){return a(),n("div",null,e)}const b=s(p,[["render",t]]);export{u as __pageData,b as default};
