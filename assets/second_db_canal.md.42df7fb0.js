...
<span class="line"><span style="color:#24292E;">}</span></span></code></pre></div><blockquote><p>经测试发现这个jar存在一些bug，ex：针对表字段，数据原本为空，修改为有值的时候，如果java这边用非String字段类型去接收会报错！</p></blockquote><h2 id="_3、canal扩展" tabindex="-1">3、Canal扩展 <a class="header-anchor" href="#_3、canal扩展" aria-label="Permalink to &quot;3、Canal扩展&quot;">​</a></h2><h3 id="mysql主备复制原理" tabindex="-1">MySQL主备复制原理 <a class="header-anchor" href="#mysql主备复制原理" aria-label="Permalink to &quot;MySQL主备复制原理&quot;">​</a></h3><p><img src="`+o+'" alt="image-20230615143944915"></p><ul><li>MySQL master 将数据变更写入二进制日志( binary log, 其中记录叫做二进制日志事件binary log events，可以通过 show binlog events 进行查看)</li><li>MySQL slave 将 master 的 binary log events 拷贝到它的中继日志(relay log)</li><li>MySQL slave 重放 relay log 中事件，将数据变更反映它自己的数据</li></ul><h3 id="canal-工作原理" tabindex="-1">canal 工作原理 <a class="header-anchor" href="#canal-工作原理" aria-label="Permalink to &quot;canal 工作原理&quot;">​</a></h3><ul><li>canal 模拟 MySQL slave 的交互协议，伪装自己为 MySQL slave ，向 MySQL master 发送dump 协议</li><li>MySQL master 收到 dump 请求，开始推送 binary log 给 slave (即 canal )</li><li>canal 解析 binary log 对象(原始为 byte 流)</li></ul>',48),t=[c];function r(E,y,i,d,u,F){return a(),n("div",null,t)}const A=s(e,[["render",r]]);export{m as __pageData,A as default};
